...
#' @description A set of functions to calculate some commonly used values when doing IRT analysis with new item types which
#' may include item bundles. IRT model currently supported are 1-3 PL, GPCM, Rasch Testlet Model, and a mix of these models.
#' Values can be computed includes the probability of correct response, item information,
...
#' @details This package computes some commonly needed results when doing dichotomous and polytomous
#' item analysis under the Item Response Theory paradigm, when tests include new item types
#' such as item bundles (testlets). \cr\cr
...
#' When there are cluster items however, values computed are based on the new model described above.
