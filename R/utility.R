####################  Utility functions (that doesn't require response data) ##########################################
## Description: 
## Compute probaility of correct response, expected scores, item info based on IRT model. 
## Model currently supported are 1-3 PL, GPCM, Rasch Testlet, and a mix of these models.
## use "what" argument to specify one or more results to return. 
## Possible results to return are:
## "prob": probablity of correct response. When Rasch testlet model is involved, this is conditional prob given both overall and each node of the nuisance dimension
## "escore": expected score
## "iteminfo: When Rasch testlet model is involved, this is item info based on marginal prob of raw scores calculated using Lord-Wingerskey algorithm
## "mprob": marginal probability of correct response for each assertion (testlet model only)
## INPUT: 
...
## 6. return_additional: if TRUE, return a list of the loglik plus some additional by-product of the function such as
# 1) probs_SA: probablity table of correct response for standalones
...
utility <- function(theta, SA_parm=NULL, Cluster_parm=NULL, Dv=1, n.nodes = 50, what = c("prob","escore","iteminfo","mprob")){
  ...
  if(is.null(SA_parm)) {
    ...
    if (nrow(SA_parm_gpc) != 0) {
      ...
      # conditional prob
      probs_SA_gpc = mapply(gpcm, rep(list(theta),nrow(SA_parm_gpc)), a.gpc, b.gpc.list, maxscr)
