...
person_fit = function(theta, SA_dat=NULL, Cluster_dat=NULL, SA_parm=NULL, Cluster_parm=NULL, Dv=1, Alpha=0.05, n.nodes = 100, n.ass.limit = 10, LW.nodes=21) {
  ...
  if(is.null(Cluster_parm) | is.null(Cluster_dat)) {
    ...
    for (k in 1:length(unique(Cluster_parm$position))) {
      ...
      prk.marginal = lord_wing(cluster_var = mvars, a = ma, b = mb, theta = mtheta, n.nodes = LW.nodes, return_additional = FALSE, Dv=Dv)
      W0 = prk.marginal[[1]]$W0[-1,]
      W1 = prk.marginal[[1]]$W1[-1,]
      W2 = prk.marginal[[1]]$W2[-1,]
      ...
      prk.marginal_new = lord_wing(cluster_var = mvars, a = ma, b = mb, theta = mtheta_new, n.nodes = LW.nodes, return_additional = FALSE, Dv=Dv)
      W0_new = prk.marginal_new[[1]]$W0[-1,]
      W1_new = prk.marginal_new[[1]]$W1[-1,]
      W2_new = prk.marginal_new[[1]]$W2[-1,]
      ...
      ### Execute the Lord and Wingersky function again for theta + small value
      prk.marginal_new2 = lord_wing(cluster_var = mvars, a = ma, b = mb, theta = mtheta_new2, n.nodes = LW.nodes, return_additional = FALSE, Dv=Dv)
      W0_new2 = prk.marginal_new2[[1]]$W0[-1,]
      W1_new2 = prk.marginal_new2[[1]]$W1[-1,]
      W2_new2 = prk.marginal_new2[[1]]$W2[-1,]
      ### Compute E(ll) new the same way as before
