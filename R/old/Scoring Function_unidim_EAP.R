####################  Person Scoring with EAP method ##########################################
...
## 8. n.nodes: number of nodes used during integration
## 9. SE: if TRUE, return standard error

# *** CATUION:
# *** This function currently only does unidimensional EAP scoring
##########################################################################################################
scoring.EAP = function(SA_dat=NULL, Cluster_dat=NULL, SA_parm=NULL, Cluster_parm=NULL, prior_mu=0, prior_var=1, Dv=1, n.nodes = 21, SE=F) {
  ...
  gq = statmod::gauss.quad.prob(n.nodes, dist = 'normal', mu = prior_mu, sigma = sqrt(prior_var))
  nodes = gq$nodes
  whts = gq$weights
  lik = as.data.frame(t(mapply(obs.data.loglik, nodes, MoreArgs = list(SA_dat=SA_dat, SA_parm=SA_parm, Dv=Dv, n.nodes=n.nodes, return_additional=F))))
  # lik = do.call(rbind, dplyr::map(nodes, obs.data.loglik, SA_dat=SA_dat, SA_parm=SA_parm, Dv=Dv, n.nodes=n.nodes, return_additional=F))
  lik = data.frame(nodes=do.call(rbind, lik$theta), loglik=do.call(rbind, lik$loglik))
  lik$lik = exp(lik$loglik)
  eap = c((lik$lik * lik$nodes) %*% whts / lik$lik %*% whts)
  
  se.numer = (lik$nodes - eap)^2 * (lik$lik * whts)  # weighted sum of squares (weighted by posterior weights)
  se = sqrt(sum(sem.numer)/(lik$lik %*% whts)) # weighted sum of squares divided by sum of the weights
  
  # se.numer<-(outer(eap,nodes,"-"))^2 * (lik$lik * whts)
  # se<-sqrt(rowSums(see.numer)/rowSums(LW))
